...
public class ProtectCameraFromWallClip : MonoBehaviour
{
	...
	// used for determeining if there is an object between the target and the camera
	public Transform pivot;
	// the point at which the camera pivots around
	public bool visualiseInEditor = true;
	// toggle for visualising the algorithm through lines for the raycast in the editor
	...
	public LayerMask clippingLayers = -1;
	//The layers of colliding objetcs


	public void setCameraMaxDistance (float x)
	{
		m_OriginalDist = Math.Abs(x);
	...
	private void Start ()
	{
		...
		pivot = m_Cam.parent;
		setCameraMaxDistance (m_Cam.localPosition.magnitude);
	...
	private void LateUpdate ()
	{
		...
		m_Ray.origin = pivot.position + pivot.forward * sphereCastRadius;
		...
		if (initialIntersect) {
			m_Ray.origin += pivot.forward * sphereCastRadius;

			// do a raycast and gather all the intersections
			m_Hits = Physics.RaycastAll (m_Ray, m_OriginalDist - sphereCastRadius, clippingLayers);
		} else {
			// if there was no collision do a sphere cast to see if there were any other collisions
			m_Hits = Physics.SphereCastAll (m_Ray, sphereCastRadius, m_OriginalDist + sphereCastRadius, clippingLayers);
		}
		...
		for (int i = 0; i < m_Hits.Length; i++) {
			// only deal with the collision if it was closer than the previous one, not a trigger, and not attached to a rigidbody
			if (m_Hits [i].distance < nearest && (!m_Hits [i].collider.isTrigger) &&
				...
				targetDist = -pivot.InverseTransformPoint (m_Hits [i].point).z;
				hitSomething = true;
			}
		}

		// visualise the cam clip effect in the editor
		if (visualiseInEditor && hitSomething) {
			Debug.DrawRay (m_Ray.origin, -pivot.forward * (targetDist + sphereCastRadius), Color.red);
		}
		...
		Vector3 temp = m_Cam.localPosition;
		temp.z = -m_CurrentDist;
		m_Cam.localPosition = temp;
	}
